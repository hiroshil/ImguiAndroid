...
bool ImguiAndroidInput::openInput(){
    ...
    jboolean tmp =  env->CallStaticBooleanMethod(gClassInfo.pJclass,gClassInfo.openInput,g_window->ID);
    if (shouldDetach) {
        gClassInfo.jvm->DetachCurrentThread();
    }
...
bool ImguiAndroidInput::closeInput(){
    ...
    LOGE("隐藏输入法");
    jboolean tmp =  env->CallStaticBooleanMethod(gClassInfo.pJclass,gClassInfo.closeInput,g_window->ID);
    if (shouldDetach) {
        gClassInfo.jvm->DetachCurrentThread();
    }
    return tmp == JNI_TRUE ? true : false;
...
//	LOGE("set.pos=%d,v=%d", pos, v);
    env->CallStaticVoidMethod(gClassInfo.pJclass, gClassInfo.io, tmp, pos, v,g_window->ID);
...
//			LOGE("无法attach.JVM线程");
    ...
    env->CallStaticVoidMethod(gClassInfo.pJclass,gClassInfo.isLongTouch,x,y,g_window->ID);
    if (shouldDetach) {
//		LOGE("准备释放jvm线程");
        gClassInfo.jvm->DetachCurrentThread();
    }
...
//			LOGE("find method mShow from JniClass");
        ...
        gClassInfo.io = env->GetStaticMethodID(gClassInfo.pJclass, "mIO", "(Ljava/lang/String;III)V");
...
//			LOGE("find method mIO from JniClass");
        ...
        gClassInfo.openInput = env->GetStaticMethodID(gClassInfo.pJclass,"openInput","(I)Z");
...
//			LOGE("find method mIO from JniClass");
        ...
        gClassInfo.closeInput = env->GetStaticMethodID(gClassInfo.pJclass,"closeInput","(I)Z");
...
//			LOGE("find method mIO from JniClass");
        ...
        gClassInfo.isLongTouch = env->GetStaticMethodID(gClassInfo.pJclass,"isLongTouch","(III)V");
